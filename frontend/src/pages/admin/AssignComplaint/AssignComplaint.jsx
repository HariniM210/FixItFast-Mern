...
const AssignComplaint = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      let lList = (lRes.data?.labours || lRes.data || []).filter(l => l.status === 'active');
      // Filter labours by admin city (case-insensitive, trimmed) to prevent cross-city options
      const norm = (s) => (s || '').toString().trim().toLowerCase();
      if (admin?.city) {
        lList = lList.filter(l => norm(l.city) === norm(admin.city));
      }
      setComplaints(cList);
      setLabours(lList);
    } catch (err) {
