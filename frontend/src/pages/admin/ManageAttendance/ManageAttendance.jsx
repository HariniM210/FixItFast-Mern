...
const ManageAttendance = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Handle city-specific messages
      if (results[0]?.data?.message) {
        if (results[0].data.message.includes('City not assigned')) {
          setError(results[0].data.message);
        }
      }
      
    } catch (err) {
      const info = apiHelpers.handleError(err);
      // Handle city assignment errors specifically
      if (err.response?.data?.requiresCityAssignment) {
        setError('City not assigned — cannot fetch attendance details.');
      } else {
  ...
  return (
    <div className="theme-page-bg" style={{ padding: 24, maxWidth: '1400px', margin: '0 auto' }}>
      <h2 className="theme-text-primary" style={{ marginBottom: 24 }}>Attendance Details</h2>
      ...
      {activeTab === 'onDuty' && (
        <div style={cardStyle}>
          ...
          ) : error && error.includes('City not assigned') ? (
            <div style={{ textAlign: 'center', color: '#dc2626', padding: '20px' }}>
              City not assigned — cannot fetch attendance details.
            </div>
          ) : currentlyOnDuty.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#6b7280', padding: '20px' }}>
              No staff currently on duty for your city.
            </div>
          ) : (
