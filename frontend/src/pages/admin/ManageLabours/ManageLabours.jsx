...
const ManageLabours = () => {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const res = await adminAPI.listLabours();
      const list = res.data?.labours || res.data || [];
      setLabours(Array.isArray(list) ? list : []);
    } catch (err) {
      const info = apiHelpers.handleError(err);
      // Normalize city-not-assigned message for display
      if ((info.message || '').toLowerCase().includes('city') && (info.message || '').toLowerCase().includes('assigned')) {
        setError('👉 City not assigned — cannot fetch labour details.');
      } else {
  ...
  const filtered = labours.filter(l => {
    ...
    return (
      ...
      (l.identityKey || '').toLowerCase().includes(q) ||
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={containerCard}>
        <div style={headerRow}>
          <h2 style={{ margin: 0, fontSize: 22 }}>Labour Details</h2>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={5} style={{ padding: 12, color: '#6b7280' }}>
                      {admin?.role === 'superadmin' ? 'No labours found.' : '👉 No labours found for your city.'}
                    </td>
                  </tr>
                )}
                {filtered.map(l => (
                  <tr key={l._id || l.id}>
                    ...
                    <td style={tdStyle}>{l.identityKey || '-'}</td>
