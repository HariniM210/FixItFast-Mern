  ...
  (error) => {
    if (error.response?.status === 401) {
      ...
      if (isAdminArea) {
        ...
        // Also clear generic auth keys used by labour login
        localStorage.removeItem('authToken');
        localStorage.removeItem('user');
        const target = '/labour/login';
        if (typeof window !== 'undefined' && window.location && path !== target) {
          window.location.href = target;
        }
      } else {
...
export const adminAPI = {
  ...
  superDashboard: () => API.get('/admin/super-dashboard'),
  listUsers: () => API.get('/admin/users'),
  listAdmins: () => API.get('/admin/admins'),
  // Admin-labour management
  listLabours: () => API.get('/admin/labours'),
  createLabour: (data) => API.post('/admin/labours', data),
