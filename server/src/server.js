...
// ---------------- CORS ----------------
const parsedEnvOrigins = (process.env.CORS_ORIGIN || process.env.FRONTEND_URL || '')
  .split(',')
  .map((o) => o.trim())
  .filter(Boolean);
...
// If running on Vercel serverless, initialize DB connection once at cold start
if (isVercel) {
  connectDatabase().catch(err => console.error('❌ Failed to init DB on Vercel:', err));
}
...
// Only start the HTTP server outside of Vercel serverless
if (!isVercel) {
  startServer();
}
