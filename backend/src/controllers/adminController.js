...
async function listLabours(req, res) {
  try {
    ...
    // If user is admin (not superadmin), filter by their city (case-insensitive)
    if (req.user.role === 'admin') {
      const admin = await Admin.findById(req.user.id).select('city');
      const adminCity = (admin && typeof admin.city === 'string') ? admin.city.trim() : '';
      if (!adminCity) {
        return res.status(400).json({ 
          success: false, 
          message: 'City not assigned — cannot fetch labour details.' 
        });
      }
      // Escape regex special characters and match whole value, case-insensitive
      const escapeRegex = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      // Match city allowing accidental surrounding whitespace in stored records
      const pattern = new RegExp(`^\\s*${escapeRegex(adminCity)}\\s*$`, 'i');
      query.city = { $regex: pattern };
    }
    // SuperAdmin can view all labours (no city filter)
    
    const labours = await Labour.find(query, 'name email phone identityKey status city createdAt')
      .sort({ createdAt: -1 })
      .limit(500);
    console.log('List labours:', { cityFilter: query.city || 'ALL', count: labours.length });
...
async function createLabour(req, res) {
  try {
    ...
    const labour = await Labour.create({ 
      ...
      role: 'labour',
      status: 'active',
      city: String(admin.city || '').trim(),
      district: String(admin.district || '').trim(),
      pincode: String(admin.pincode || '').trim(),
      createdBy: adminId
    });
