...
// Static method to get current on-duty labours filtered by city
attendanceSchema.statics.getCurrentlyOnDutyByCity = async function(city) {
  try {
    // Get latest entry for each labour from specific city
    const pipeline = [
      {
        $sort: { labour: 1, timestamp: -1 }
      },
      ...
      {
        $match: {
          "labour.city": { $regex: new RegExp(`^${city}$`, 'i') } // Case-insensitive match
        }
      },
      {
        $project: {
