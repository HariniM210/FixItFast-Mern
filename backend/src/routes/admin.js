...
router.get('/complaints/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const complaint = await Complaint.findById(req.params.id)
      .populate('user', 'name email phone city district pincode createdAt')
...
router.put('/complaints/:id/assign', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Verify labour is from same city as admin (case-insensitive, trimmed)
    const norm = (s) => (s || '').toString().trim().toLowerCase();
    if (norm(labour.city) !== norm(admin.city)) {
    ...
    // Check if labour has checked in today (allow assignment regardless, but include info)
    const attendanceStatus = await Attendance.getCurrentStatus(labourId);
    const notCheckedIn = !attendanceStatus || attendanceStatus.status !== 'check_in';
    
    // Proceed with assignment even if not checked in; add note in status history
    ...
    complaint.statusHistory.push({
      ...
      note: `Assigned to ${labour.name} (${activeTaskCount + 1}/4 tasks)${notCheckedIn ? ' — Labour not checked in yet' : ''}`
    });
...
router.get('/attendance/on-duty', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Check if admin has city assigned (skip for superadmin)
    if (!admin.city && admin.role !== 'superadmin') {
      return res.status(400).json({ 
        success: false, 
        message: 'City not assigned — cannot fetch attendance details.',
        requiresCityAssignment: true
      });
    }
    
    // SuperAdmin can see all, regular admin sees only their city
    const onDutyLabours = admin.role === 'superadmin' 
      ? await Attendance.getCurrentlyOnDuty()
      : await Attendance.getCurrentlyOnDutyByCity(admin.city);
...
router.get('/attendance/labour-status', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // SuperAdmin can see all labours, regular admin sees only their city labours
    const labourQuery = admin.role === 'superadmin' 
      ? { status: 'active' }
      : { 
          status: 'active',
          city: { $regex: new RegExp(`^${admin.city}$`, 'i') } // Case-insensitive match
        };
    
    const allLabours = await Labour.find(labourQuery)
      .select('name email phone skills city').lean();
...
router.get('/attendance', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Get labours based on admin role
    const labourQuery = admin.role === 'superadmin' 
      ? { status: 'active' }
      : { 
          status: 'active',
    ...
    let cityLabours = await Labour.find(labourQuery).select('_id name').lean();
    
    if (cityLabours.length === 0) {
      const message = admin.role === 'superadmin' 
        ? 'No attendance records found.'
        : 'No attendance records found for your city.';
      return res.json({
        success: true,
        attendance: [],
        message: message,
    ...
    // Build query - start with filtered labour IDs
    let labourIds = cityLabours.map(l => l._id);
    let query = { labour: { $in: labourIds } };
    
    // Filter by labour name if provided (within city-filtered labours)
    if (labourName && labourName.trim()) {
      const filteredLabours = cityLabours.filter(l => 
        l.name.toLowerCase().includes(labourName.trim().toLowerCase())
      );
      
      if (filteredLabours.length > 0) {
        labourIds = filteredLabours.map(l => l._id);
        query.labour = { $in: labourIds };
      } else {
        // No matching labours found in this city
        return res.json({
          success: true,
          attendance: [],
          message: `No labours found matching '${labourName}' in your city.`,
          adminCity: admin.city,
          pagination: {
            currentPage: parseInt(page),
            totalPages: 0,
...
router.get('/attendance/report', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const labours = await Labour.find(labourQuery)
      .select('_id name email employeeId department city')
      .lean();
